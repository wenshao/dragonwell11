...
public final class HexDigits {
    ...
    /**
     * put a little-endian packed integer for the 4 ASCII bytes for an input unsigned 2-byte integer.
     ...
     */
    public static void putDigits(byte[] buf, int charPos, int b0, int b1) {
        UNSAFE.putIntUnaligned(
                buf,
                Unsafe.ARRAY_BYTE_BASE_OFFSET + charPos,
                DIGITS[b0 & 0xff] | (DIGITS[b1 & 0xff] << 16),
                false);
    }

    /**
     * put a little-endian packed long for the 8 ASCII bytes for an input unsigned 4-byte integer.
     * {@code b0} is the most significant byte and {@code b3} is the least significant byte.
     * The integer is passed byte-wise to allow reordering of execution.
     */
    public static void putDigits(byte[] buf, int charPos, int b0, int b1, int b2, int b3) {
